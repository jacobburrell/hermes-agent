...
| `HERMES_GATEWAY_BUSY_ACK_ENABLED` | Legacy compatibility fallback for whether the gateway sends an acknowledgment (⚡/⏳/⏩) when a user sends input while the agent is busy. Explicit `display.chat_types.*.busy_ack_enabled`, `display.platforms.*.busy_ack_enabled`, and `display.busy_ack_enabled` values take precedence, in that order. Use `config.yaml` for new configuration. |
