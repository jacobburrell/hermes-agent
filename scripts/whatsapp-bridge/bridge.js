...
// A private bridge-to-adapter capability, generated by the spawning adapter
// for each bridge process.  It is not a user config value, is never logged or
// returned by /health, and is deleted from Node's inherited environment after
// bootstrap.  A manually started bridge still fails closed: it generates an
// unknown capability, so no unauthenticated caller can materialize media.
const _inheritedMaterializationCapability = process.env.HERMES_INTERNAL_WHATSAPP_BRIDGE_CAPABILITY;
delete process.env.HERMES_INTERNAL_WHATSAPP_BRIDGE_CAPABILITY;
const INBOUND_MEDIA_MATERIALIZATION_CAPABILITY = (
  typeof _inheritedMaterializationCapability === 'string'
  && _inheritedMaterializationCapability.length >= 32
) ? _inheritedMaterializationCapability : randomBytes(32).toString('base64url');
const INBOUND_MEDIA_MATERIALIZATION_HEADER = 'x-hermes-whatsapp-materialize-capability';
...
// Raw Baileys media messages never cross /messages.  This small process-local
// store is intentionally not durable: it only supports the one authenticated
// materialization after Python has admitted the event as OPERATE.
const inboundMediaStore = createBoundedInboundMediaStore({ limit: 128, ttlMs: 120000 });
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // First stage is metadata/caption only.  Do not contact WhatsApp's CDN
      // or write a cache file until the Python adapter classifies this event.
      ...
      if (event.hasMedia) {
        // Origin validation happens inside the bounded store against
        // ``msg.key``; invalid/malformed pairs are never materializable.
        inboundMediaStore.remember({
          chatId,
          messageId: event.messageId,
          msg,
          context: { chatId, senderId, senderNumber, botIds, isGroup },
        });
      }
      messageQueue.push(event);
      emitDebugEvent({
...
// Materialize exactly one raw inbound media message after the Python adapter
// has admitted it.  This is not a generic download endpoint: the request is
// capability-authenticated and bound to a single retained chat/message pair.
const materializeInboundMedia = createInboundMediaMaterializer({
  capability: INBOUND_MEDIA_MATERIALIZATION_CAPABILITY,
  inboundMediaStore,
  materialize: async (entry) => {
    ...
    // Return only the materialized transport fields.  In particular, no raw
    // Baileys message, nativeMetadata, read-receipt key, cache directory, or
    // capability is serialised into this HTTP response.
    return {
...
app.post('/materialize-inbound-media', async (req, res) => {
  const result = await materializeInboundMedia({
    suppliedCapability: req.get(INBOUND_MEDIA_MATERIALIZATION_HEADER),
    chatId: req.body?.chatId,
    messageId: req.body?.messageId,
  });
  if (!result.ok) {
    // Do not disclose whether a particular chat/message exists; callers only
    // learn that this one-shot materialization is unavailable.
    return res.status(result.status).json({ error: 'Inbound media unavailable' });
  }
