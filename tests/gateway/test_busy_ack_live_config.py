...
@pytest.mark.asyncio
async def test_real_whatsapp_seven_image_burst_falls_back_to_silent_queue(
    tmp_path,
    monkeypatch,
):
    """One owner plus six non-steerable images reproduce the observed burst."""
    import gateway.run as gateway_run
    ...
    async def _blocking_owner(_event):
        owner_started.set()
...
async def test_multiplex_busy_ack_reads_source_profile_without_cross_profile_bleed(
    ...
    runner._resolve_profile_home_for_source = lambda source: (
        secondary_home if source.profile == "secondary" else primary_home
    )
...
def test_platform_override_wins_over_global_setting(tmp_path, monkeypatch):
    import gateway.run as gateway_run
...
def test_valid_false_then_malformed_yaml_keeps_last_known_good(
    tmp_path,
    monkeypatch,
):
    import gateway.run as gateway_run
...
def test_first_malformed_whatsapp_config_fails_closed(tmp_path, monkeypatch):
    import gateway.run as gateway_run
...
@pytest.mark.parametrize("document", ["- list-item\n", "scalar\n", "null\n"])
def test_first_non_mapping_whatsapp_config_fails_closed(
    tmp_path,
    monkeypatch,
    document,
):
...
def test_managed_busy_ack_policy_is_valid_alone_and_wins_user_yaml(
    tmp_path,
    monkeypatch,
):
    import gateway.run as gateway_run
...
def test_malformed_multiplex_profile_does_not_bleed_from_valid_peer(
    ...
    primary = _event(profile="primary")
...
def test_empty_mapping_preserves_env_fallback(
    tmp_path,
    monkeypatch,
    env_value,
    expected,
